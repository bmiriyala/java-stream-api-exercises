...
//                              .findFirst();
	...
	public void exercise9() {
		...
		OptionalDouble result = orderRepo.findAll()
				...
				.average();
				//.getAsDouble();
		double endResult = result.isPresent()? result.getAsDouble() : result.orElse(0);

		long endTime = System.currentTimeMillis();
		log.info(String.format("exercise 9 - execution time: %1$d ms", (endTime - startTime)));
		log.info("Average = " + endResult);
	}
	...
	@Test
	@DisplayName("Obtain a data map with order id and its total price (using reduce)")
